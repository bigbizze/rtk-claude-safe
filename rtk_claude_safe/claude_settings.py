...
def _patch_bash_matcher(matcher_entry: dict[str, Any]) -> bool:
    """Replace bare rtk hooks under this matcher with the scoped list."""
    hooks = matcher_entry.get("hooks")
    if not isinstance(hooks, list):
        matcher_entry["hooks"] = build_claude_scoped_hooks(RTK_COMMAND)
        return True
    ...
    kept.extend(build_claude_scoped_hooks(RTK_COMMAND))
...
def patch_settings(path: Path = DEFAULT_SETTINGS_PATH) -> bool:
    ...
    if bash_entry is None:
        bash_entry = {"matcher": "Bash", "hooks": build_claude_scoped_hooks(RTK_COMMAND)}
