    ...
    for group in data["hooks"]["PreToolUse"]:
        if isinstance(group, dict) and group.get("matcher") == "Bash":
            entries.extend(
                ...
                if isinstance(hook, dict) and hook.get("command") == RTK_COMMAND
            )
    return entries
...
def test_claude_settings_create_scoped_hooks_from_scratch(tmp_path) -> None:
    ...
    assert patch_settings(settings_path)

    data = _read(settings_path)
    assert data["hooks"]["PreToolUse"] == [
        {"matcher": "Bash", "hooks": build_claude_scoped_hooks(RTK_COMMAND)}
    ]
    assert not patch_settings(settings_path)
...
def test_claude_settings_remove_rtk_hooks_from_duplicate_bash_groups(tmp_path) -> None:
    ...
    rtk_entries = _rtk_entries(data)
    assert len(rtk_entries) == len(SCOPED_PATTERNS)
    assert rtk_entries == build_claude_scoped_hooks(RTK_COMMAND)
...
def test_claude_settings_reconcile_stale_scoped_hooks(tmp_path) -> None:
    ...
    assert hooks[1:] == build_claude_scoped_hooks(RTK_COMMAND)
